            ...
            for schema in schemas:
                ...
                for t in schema_types:
                    ...
                    if t == "WebSite":
                        ...
                        # Flag as invalid when the schema has offers.price="0" or
                        # no applicationCategory — signature of the wrong-type mistake
                        offers = schema.get("offers", {})
                        if isinstance(offers, list):
                            offers = offers[0] if offers else {}
                        price_zero = str(offers.get("price", "")).strip() == "0"
                        no_app_category = not schema.get("applicationCategory")
                        if price_zero or no_app_category:
                            result.webapp_invalid = True
                    elif t == "FAQPage":
                        result.has_faq = True
