...
def build_recommendations(
    ...
    # v4.10: buyer-facing vertical readiness recommendations
    if vertical_profile is not None and vertical_profile.checked:
        recommendations.extend(vertical_profile.priority_actions[:3])
...
def _build_audit_result(
    ...
    vertical_profile=None,  # v4.10: buyer-facing vertical profile
    perf=None,  # v4.12: Core Web Vitals / performance signals
    links=None,  # v4.12: broken link + 404 detection
    conversion=None,  # v4.12: conversion readiness checklist
    tracking=None,  # v4.13: analytics / tracking presence
    indexability=None,  # v4.14: indexability conflict detection
    freshness=None,  # v4.14: freshness signals
    indexnow=None,   # v4.10: IndexNow protocol readiness
    aeo=None,        # v4.10: Answer Engine Optimization
    sxo=None,        # v4.10: Search Experience Optimization
) -> AuditResult:
    ...
    # v4.10: Vertical Market Readiness
    effective_vertical_profile = vertical_profile if vertical_profile is not None else VerticalAuditResult()

    # v4.12: Performance / Core Web Vitals
    if perf is not None:
        effective_perf = perf
    elif soup is not None:
        from geo_optimizer.core.audit_perf import audit_perf

        effective_perf = audit_perf(soup)
    else:
        effective_perf = PerfResult()

    # v4.12: Broken links — only when soup is available; skipped in batch mode
    # (network-intensive — 40 HEAD requests per page)
    if links is not None:
        effective_links = links
    else:
        # Run link checks only when explicitly triggered via run_full_audit;
        # _build_audit_result callers that pass links=None get an empty result
        effective_links = LinksResult()

    # v4.12: Conversion readiness
    if conversion is not None:
        effective_conversion = conversion
    elif soup is not None:
        from geo_optimizer.core.audit_conversion import audit_conversion

        effective_conversion = audit_conversion(soup, base_url, schema=schema, meta=meta, content=content)
    else:
        effective_conversion = ConversionResult()

    # v4.13: Analytics / tracking presence
    if tracking is not None:
        effective_tracking = tracking
    elif soup is not None:
        from geo_optimizer.core.audit_tracking import audit_tracking

        effective_tracking = audit_tracking(soup, raw_html="", base_url=base_url)
    else:
        effective_tracking = TrackingResult()

    # v4.14: Indexability conflict detection — needs soup + response headers
    effective_indexability = indexability if indexability is not None else IndexabilityResult()

    # v4.14: Freshness signals
    effective_freshness = freshness if freshness is not None else FreshnessResult()

    # IndexNow / AEO / SXO
    from geo_optimizer.models.results import IndexNowResult, AEOResult, SXOResult
    effective_indexnow = indexnow if indexnow is not None else IndexNowResult()
    effective_aeo = aeo if aeo is not None else AEOResult()
    effective_sxo = sxo if sxo is not None else SXOResult()
    ...
    recommendations = build_recommendations(
        ...
        effective_vertical_profile,
    )
    from geo_optimizer.core.action_intelligence import build_next_actions

    next_actions = build_next_actions(breakdown, recommendations)
    ...
    # v4.14: Indexability conflict recommendations
    if effective_indexability.checked:
        if effective_indexability.has_nosnippet:
            recommendations.append(
                "⚠️ CRITICAL: nosnippet directive detected — Google explicitly documents this as "
                "blocking content from AI Overviews and AI Mode. "
                f"Remove it from: {effective_indexability.conflict_detail or 'meta robots / X-Robots-Tag'}."
            )
        elif effective_indexability.robots_meta_conflict:
            recommendations.append(
                f"⚠️ Indexability conflict: {effective_indexability.conflict_detail}"
            )

    # v4.14: Freshness conflict recommendations
    if effective_freshness.checked and effective_freshness.date_conflict:
        recommendations.append(
            f"Freshness conflict: {effective_freshness.conflict_detail}"
        )

    # v4.14: llms.txt coverage gap recommendations
    if llms.found and llms.important_pages_missing:
        missing = ", ".join(f"/{p}" for p in llms.important_pages_missing)
        recommendations.append(
            f"llms.txt is missing links to important pages: {missing}. "
            "Add these to the relevant H2 sections so AI systems can discover your key content."
        )

    # v4.14: Heading ID recommendations
    if content.heading_id_ratio < 0.5 and content.heading_count > 2:
        recommendations.append(
            "Add id= attributes to H2/H3 headings to enable section-level citations "
            f"(currently {int(content.heading_id_ratio * 100)}% of headings have anchors). "
            "Example: <h2 id=\"pricing\">Pricing</h2>"
        )

    # Text-to-HTML ratio warning (< 10% = boilerplate-heavy / thin)
    if getattr(content, "text_html_ratio", 0.0) > 0 and content.text_html_ratio < 0.10:
        recommendations.append(
            f"Low text-to-HTML ratio ({content.text_html_ratio:.1%}): less than 10% of page "
            "bytes are visible text. Reduce template/nav/script overhead — AI engines may "
            "treat heavily bloated pages as thin content."
        )

    # External nofollow warning
    if getattr(content, "all_external_nofollow", False):
        recommendations.append(
            "⚠️ All outgoing external links carry rel=\"nofollow\" — AI citation engines cannot "
            "follow these references to verify sourcing credibility. Remove nofollow from links "
            "to authoritative sources (studies, official docs, data providers)."
        )
    elif getattr(content, "external_nofollow_count", 0) > 0:
        total_ext = content.external_links_count
        nf = content.external_nofollow_count
        recommendations.append(
            f"{nf} of {total_ext} external link{'s' if total_ext != 1 else ''} "
            "carry rel=\"nofollow\". Consider removing nofollow from links to authoritative "
            "sources so AI crawlers can follow and validate your citations."
        )

    # v4.14: Canonical-redirect conflict (from links result)

    # AEO recommendations
    if effective_aeo.checked:
        recommendations.extend(effective_aeo.recommendations)

    # SXO recommendations
    if effective_sxo.checked:
        recommendations.extend(effective_sxo.recommendations)
    ...
    result = AuditResult(
        ...
        vertical_profile=effective_vertical_profile,
        next_actions=next_actions,
        perf=effective_perf,
        links=effective_links,
        conversion=effective_conversion,
        tracking=effective_tracking,
        indexability=effective_indexability,
        freshness=effective_freshness,
        indexnow=effective_indexnow,
        aeo=effective_aeo,
        sxo=effective_sxo,
    )
...
def run_full_audit(
    ...
    llms = _audit_llms_from_response(r_llms, r_full=r_llms_full, base_url=base_url)
    ...
    # v4.10: market-facing vertical profile layer
    from geo_optimizer.core.vertical_profile import audit_vertical_profile

    vertical_profile_result = audit_vertical_profile(
        soup=soup,
        base_url=base_url,
        schema=schema,
        meta=meta,
    ...
    # v4.12: Broken link audit — run before _build_audit_result so it can be passed in
    from geo_optimizer.core.audit_links import audit_links

    links_result = audit_links(soup, base_url, check_external=check_external_links)

    # v4.14: Indexability conflict detection — uses soup + response headers
    from geo_optimizer.core.audit_indexability import audit_indexability

    indexability_result = audit_indexability(
        soup=soup,
        response_headers=resp_headers,
        robots_allowed=robots.citation_bots_ok,
    )

    # v4.14: Freshness signals — uses soup + response headers + schema
    from geo_optimizer.core.audit_freshness import audit_freshness

    freshness_result = audit_freshness(
        soup=soup,
    ...
    result = _build_audit_result(
        ...
        vertical_profile=vertical_profile_result,
        links=links_result,
