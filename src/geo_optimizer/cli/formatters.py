...
def format_audit_json(result: AuditResult) -> str:
    ...
    # v4.10: Vertical market profile
    if hasattr(result, "vertical_profile") and result.vertical_profile and result.vertical_profile.checked:
        data["vertical_profile"] = asdict(result.vertical_profile)

    if hasattr(result, "next_actions") and result.next_actions:
        data["next_actions"] = [asdict(item) for item in result.next_actions]
...
def format_audit_text(result: AuditResult) -> str:
    ...
    if result.ai_discovery:
        ...
        if result.ai_discovery.has_service:
            lines.append("  ✅ /ai/service.json valid")
        else:
            lines.append("  ⚠️  /ai/service.json missing or invalid")
    ...
    # Vertical market profile (v4.10)
    vp = getattr(result, "vertical_profile", None)
    if vp and vp.checked:
        lines.append("")
        lines.append(_section_header("19. VERTICAL MARKET READINESS"))
        lines.append(
            f"  Vertical: {vp.vertical} (detected: {vp.detected_vertical}, confidence: {vp.detection_confidence:.2f})"
        )
        lines.append(f"  Locale: {vp.market_locale}")
        lines.append(f"  Business readiness: {vp.business_readiness_score}/100")
        lines.append(
            f"  Trust: {vp.trust_signals}/5 | Conversion: {vp.conversion_signals}/5 | "
            f"Local: {vp.locality_signals}/5 | Vertical: {vp.vertical_signals}/5"
        )
        if vp.market_locale == "en-fr":
            lines.append(f"  Bilingual readiness: {'✅' if vp.bilingual_ready else '❌'}")
        for action in vp.priority_actions[:3]:
            lines.append(f"  💡 {action}")

    # Action intelligence roadmap (v4.11)
    next_actions = getattr(result, "next_actions", None)
    if next_actions:
        lines.append("")
        lines.append(_section_header("20. NEXT BEST ACTIONS"))
        for item in next_actions[:5]:
            lines.append(
                f"  • {item.title} [{item.priority}] impact={item.impact} effort={item.effort} "
                f"(+{item.expected_score_gain})"
            )
