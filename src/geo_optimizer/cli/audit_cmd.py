...
@click.option(
    ...
    default="auto",
    show_default=True,
    help="Apply a vertical market profile for buyer-facing recommendations.",
)
@click.option(
    "--market-locale",
    ...
    default="en",
    show_default=True,
    help="Market locale profile used by vertical readiness checks.",
)
def audit(
    url,
    sitemap,
    ...
    if verbose:
        ...
        # Keep default output concise in normal runs.
    ...
    try:
        if sitemap:
            ...
            if _use_spinner:
                ...
                # Auto-detect output language from the audited site
                from geo_optimizer.i18n import auto_detect_lang
                _html_lang = getattr(getattr(result, "signals", None), "lang_value", "") or ""
    ...
    if output_file:
        ...
        # When JSON is written to disk, still provide a direct terminal summary
        # so users don't need to open the file for the primary outcome.
        if output_format == "json" and not sitemap:
            click.echo(f"📊 GEO score: {result.score}/100 ({result.band})")
            if getattr(result, "vertical_profile", None) and result.vertical_profile.checked:
                vp = result.vertical_profile
                click.echo(
                    "🏷️  Vertical: "
                    f"{vp.vertical} (detected: {vp.detected_vertical}, confidence: {vp.detection_confidence:.2f})"
                )
            if result.recommendations:
                click.echo(f"💡 Top action: {result.recommendations[0]}")
    else:
        click.echo(output)
