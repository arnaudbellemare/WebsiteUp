...
@click.option(
    "--only",
    default=None,
    help="Filter categories: robots,llms,schema,meta,ai_discovery,content,vertical (comma-separated)",
)
...
@click.option(
    ...
    default="auto",
    show_default=True,
    help="Vertical profile for buyer-facing fix priorities.",
)
@click.option(
    "--market-locale",
    ...
    default="en",
    show_default=True,
    help="Market locale profile for conversion/trust recommendations.",
)
def fix(url, output_dir, dry_run, do_apply, only, config_file, vertical, market_locale):
    ...
    if only:
        ...
        valid_categories = {"robots", "llms", "schema", "meta", "ai_discovery", "content", "vertical"}
