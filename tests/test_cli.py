...
class TestCLIVersionAndHelp:
    ...
    def test_help_lists_all_commands(self, runner):
        ...
        assert "visibility-audit" in result.output
        assert "scanner" in result.output
        assert "scorer" in result.output
        assert "citation-optimizer" in result.output
        assert "fixer" in result.output
        assert "presence-monitor" in result.output
        assert "readiness-monitor" in result.output
        assert "rivalry" in result.output
    ...
    def test_autopilot_help(self, runner):
        """geo autopilot --help shows autonomous loop options."""
    ...
    def test_apply_help(self, runner):
        """geo apply --help shows stack-aware apply options."""
    ...
    def test_rivalry_help(self, runner):
        """geo rivalry --help shows A/B/AB workflow options."""
        result = runner.invoke(cli, ["rivalry", "--help"])
        assert result.exit_code == 0
        assert "--url" in result.output
        assert "--output" in result.output
...
class TestFormatters:
    ...
    def test_format_audit_json_vertical_profile(self, sample_audit_result):
        """JSON output includes vertical profile when enabled."""
        from geo_optimizer.cli.formatters import format_audit_json

        sample_audit_result.vertical_profile = VerticalAuditResult(
            checked=True,
            vertical="real-estate-proptech",
            detected_vertical="real-estate-proptech",
            detection_confidence=0.81,
            market_locale="en-fr",
            trust_signals=3,
            conversion_signals=2,
            locality_signals=4,
            vertical_signals=3,
            bilingual_ready=True,
            business_readiness_score=78,
            signals=[
                VerticalSignal(key="trust_proof", label="Trust proof", detected=True, evidence=["reviews", "insured"])
            ],
            priority_actions=["Add bilingual service pages"],
        )
        data = json.loads(format_audit_json(sample_audit_result))
        assert "vertical_profile" in data
        assert data["vertical_profile"]["vertical"] == "real-estate-proptech"
        assert data["vertical_profile"]["business_readiness_score"] == 78

    def test_format_audit_text_vertical_profile(self, sample_audit_result):
        """Text output includes the vertical market readiness section."""
        from geo_optimizer.cli.formatters import format_audit_text

        sample_audit_result.vertical_profile = VerticalAuditResult(
            checked=True,
            vertical="legal-professional-services",
            detected_vertical="legal-professional-services",
            ...
            locality_signals=1,
            vertical_signals=3,
            business_readiness_score=66,
            priority_actions=["Add practice-area pages"],
        )
        output = format_audit_text(sample_audit_result)
        assert "VERTICAL MARKET READINESS" in output
        assert "legal-professional-services" in output
